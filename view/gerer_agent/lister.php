...
$edit_societe_id = isset($_GET['edit_id']) ? $_GET['edit_id'] : null;
    ...
    if (isset($_POST['submit'])) {
        ...
        if (empty($id_agent)) {
            ...
            $sql = "UPDATE `agent` SET `nom_agent`='$nom',`prenom_agent`='$prenom',`fonction_agent`='$fonction',`contact_agent`='$contact' WHERE id_agent='$id_agent'";
            $result = mysqli_query($conn, $sql);
...
<body>
        ...
        <table class="table table-hover text-center">
            <thead class="table-dark">
                <tr>
                    ...
                    <th scope="col">Prénom(s) de l'agent</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php echo $row['fonction_agent'] ?></td>
                    <td><?php echo $row['contact_agent'] ?></td>
                    <td>
                        <a href="#" class="link-dark" onclick="openModal(<?php echo $row['id_agent']?>)"><i
                                class="fa-solid fa-pen-to-square me-3"></i></a>
                        <a href="#" class="link-dark" onclick="confirmerSuppression(<?php echo $row['id_agent']?>)"><i
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="" method="post">
                        <div class="mb-3">
                            <label for="nom" name="nom" class="col-form-label">Nom de l'agent:</label>
                            <input type="text" class="form-control" name="nom" id="nom" placeholder="Nom de l'agent"
                                required style="font-size:90%">
                            <input type="hidden" id="id" name="id">
                        </div>
                        <div class="mb-3">
                            <label for="prenom" name="prenom" class="col-form-label">Prénom de l'agent:</label>
                            <input type="text" class="form-control" name="prenom" id="prenom"
                                placeholder="Préom de l'agent" required style="font-size:90%">
                        </div>
                        <div class="mb-3">
                            <label for="fonction" name="fonction" class="col-form-label">Fonction:</label>
                            <select class="form-select" name="fonction" id="fonction" placeholder="Choisir ..."
                                ...
                                <option value="Officier de Police">Officier de Police</option>
                            </select>
                            <div>
                                <div class="mb-3">
                                    <label for="contact" name="contact" class="col-form-label">Contact:</label>
                                    <input type="number" class="form-control" name="contact" id="contact"
                                        placeholder="contact de l'agent" required style="font-size:90%">
                                </div>
    ...
    function openModal(edit_id = null) {
        ...
        if (edit_id) {
            ...
            document.getElementById('staticBackdropLabel').innerText = 'Modifier agent';
            var id = edit_id;
            ...
            function getDataById(id) {
                $.ajax({
                    ...
                    success: function(data) {
                        $('#id').val(data.id_agent);
                        $('#nom').val(data.nom_agent);
                        $('#prenom').val(data.prenom_agent);
                        $('#fonction').val(data.fonction_agent);
                        $('#contact').val(data.contact_agent);
                    },
                    error: function(xhr, status, error) {
            ...
            document.getElementById('staticBackdropLabel').innerText = 'Nouveau agent';
