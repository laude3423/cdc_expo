...
<body>
    ...
    <div id="myModal" class="modal">
        <div class="modal-content">
            ...
            <p>Bonjour!</p>
            <p>Ce Procès-Verbal de Scellage est délivré par le Ministère des Mines.</p>
        </div>
    </div>
    ...
    <div id="fileLink" style="display:none;">
        ...
        if(isset($_GET['id_data_cc'])) {
            $id = $_GET['id_data_cc'];
            $sql = "SELECT lien_pv_scellage FROM data_cc WHERE id_data_cc=$id";
            $result = mysqli_query($conn, $sql);
            $row = mysqli_fetch_assoc($result);
            if(!empty($row['lien_pv_scellage'])) {
                $lien_pv_scellage = $row["lien_pv_scellage"];
                 $pdfFilePath = $lien_pv_scellage;
