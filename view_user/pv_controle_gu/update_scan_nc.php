...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
       ...
       if (isset($_FILES['scan_ov_droit']) && $_FILES['scan_ov_droit']['error'] == UPLOAD_ERR_OK) {
        ...
        $sql = "UPDATE `data_cc` SET  `droit_conformite` ='$droit_conformite',`validation_chef`='En attente', `validation_directeur`='En attente', `num_ov` ='$num_ov',`date_ov` ='$date_ov',`num_quittance` ='$num_quittance',`date_quittance` ='$date_quittance' WHERE id_data_cc='$id'";
            $result = mysqli_query($conn, $sql);
