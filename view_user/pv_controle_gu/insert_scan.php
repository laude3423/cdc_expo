...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
       ...
       // Directory for upload
        ...
        // Define the output filenames for the combined files
        ...
        if ((isset($_FILES['scan_quittance_rdv']) && $_FILES['scan_quittance_rdv']['error'] == UPLOAD_ERR_OK) ||
            ...
            $numQTT_cleaned = preg_replace('/[^a-zA-Z0-9]/', '-', $num_quittance);
            $fileName_QT = "SCAN_QTT_".$numQTT_cleaned.".pdf";
            $uploadPath_QT = $uploadDir . $fileName_QT;
        ...
        $uploadPath_CTL = $uploadDir . $fileName_CTL;
            ...
            $sql = "UPDATE `data_cc` SET `redevance` ='$redevance', `ristourne` ='$ristourne', `droit_conformite` ='$droit_conformite', `num_ov` ='$num_ov', `num_ov` ='$num_ov',`description` ='$description',`num_quittance` ='$num_quittance',`date_quittance` ='$date_quittance',`scan_controle`='$uploadPath_CTL',`scan_scellage`='$uploadPath_SCE' WHERE id_data_cc='$id'";
            $result = mysqli_query($conn, $sql);
