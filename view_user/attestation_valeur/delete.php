...
if (isset($_POST['id'])) {
    $id = $_POST['id'];
    $sql = "SELECT * FROM contenu_attestation WHERE id_contenu_attestation=?";
    ...
    $query = "DELETE FROM contenu_attestation WHERE id_contenu_attestation = ?";
    ...
    if ($stmt === false) {
        ...
        if ($stmt->execute()) {
            ...
            $stmt3 = $conn->prepare("UPDATE contenu_attestation SET `validation_contenu` = 'En attente' WHERE id_data_cc = ?");
            if ($stmt3) {
