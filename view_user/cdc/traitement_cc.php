...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $activite = "Insertion d'un nouveau certificat de conformité.";
       ...
       // include '../generate_fichier/generate_insert_controle.php';
        ...
        //pj_DOM
        if (($_FILES['pj_domciliation']) && $_FILES['pj_domciliation']['error'] == UPLOAD_ERR_OK) {
            $numDom_cleaned = preg_replace('/[^a-zA-Z0-9]/', '-', $num_domiciliation);
            $uploadDir = '../upload/';
            $fileName_DOM = "SCAN_DOM_" .$numDom_cleaned.$heureExacte_cleaned.".". pathinfo($_FILES['pj_domciliation']['name'],
            PATHINFO_EXTENSION);
        ...
        $bind_result = mysqli_stmt_bind_param($stmt, 'ssssi',     // string
