...
$edit_societe_id = isset($_GET['edit_id']) ? $_GET['edit_id'] : null;
    ...
    if (isset($_POST['submit'])) {
        ...
        $destination_vol = $_POST['destination_vol'];
        $id_vol = $_POST['id'];

        if (empty($id_vol)) {
            // Insertion d'une nouvelle société
            $sql = "INSERT INTO `vol`(`numero_vol`, `nom_compagnie`, `destination_vol`) VALUES ('$nom', '$nom_compagnie', $destination_vol)";
            ...
            $sql = "UPDATE `vol` SET `numero_vol`='$nom', `nom_compagnie`='$nom_compagnie',`destination_vol`='$destination_vol' WHERE id_continent='$id_continent'";
            $result = mysqli_query($conn, $sql);
...
<body>
    <div class="container">
        ...
        <table id="agentTable" class="table table-hover text-center">
            ...
            <tbody>
                ...
                <tr>
                    <td>✅</td>
                    <td><?php echo $row['numero_vol'] ?></td>
                    <td><?php echo $row['nom_compagnie'] ?></td>
                    <td><?php echo $row['destination_vol'] ?></td>
                    <td>
                        <a href="#" class="link-dark" onclick="openModal(<?php echo $row['id_vol']?>)"><i
                                class="fa-solid fa-pen-to-square me-3"></i></a>
                        <a href="#" class="link-dark" onclick="confirmerSuppression(<?php echo $row['id_vol']?>)"><i
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="staticBackdropLabel">Nouvel vol</h1>
                ...
                <div class="modal-body">
                    <form action="" method="post">
                        <div class="mb-3">
                            <label for="nom" name="nom" class="col-form-label">Numéro du vol:</label>
                            <input type="text" class="form-control" name="nom" id="nom" placeholder="Numéro du vol"
                                required style="font-size:90%">
                            <input type="hidden" id="id" name="id">
                        </div>
                        <div class="mb-3">
                            <label for="nom_compagnie" name="nom_compagnie" class="col-form-label">Nom de la
                                compagnie:</label>
                            <input type="text" class="form-control" name="nom_compagnie" id="nom_compagnie"
                                placeholder="Nom de la compagnie" required style="font-size:90%">
                        </div>
                        <div class="mb-3">
                            <label for="destination_vol" name="destination_vol" class="col-form-label">Destination du
                                vol:</label>
                            <input type="text" class="form-control" name="destination_vol" id="destination_vol"
                                placeholder="Destination du vol" required style="font-size:90%">
                        </div>
    ...
    function openModal(edit_id = null) {
        ...
        if (edit_id) {
            ...
            document.getElementById('staticBackdropLabel').innerText = 'Modifier un vol';
            var id = edit_id;
            ...
            function getDataById(id) {
                $.ajax({
                    ...
                    success: function(data) {
                        $('#id').val(data.id_vol);
                        $('#nom').val(data.numero_vol);
                        $('#nom_compagnie').val(data.nom_compagnie);
                        $('#destination_vol').val(data.destination_vol);
                    },
                    error: function(xhr, status, error) {
            ...
            document.getElementById('staticBackdropLabel').innerText = 'Nouvel vol';
            document.getElementById('nom').value = '';
            document.getElementById('nom_compagnie').value = '';
            document.getElementById('destination_vol').value = '';
        }
