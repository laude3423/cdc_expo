<?php
    ...
    if (isset($_GET['id'])) {
        $id = $_GET['id'];
        $sql = "SELECT vi.*,dcc.*, vo.*, agent.*, fr.* FROM `visa` AS vi 
    ...
    <div class="info  container">
        ...
        <div class="info1">
            <div class="alert alert-light" role="alert">
                ...
                <p><strong>Destination finale:</strong> <?php echo $row2['pays_destination']; ?></p>
            </div>
            <div class="alert alert-light" role="alert">
                <h5 id="list-item-1">Information sur la facture</h5>
                <hr>
                <p><strong>Numéro de la facture:</strong> <?php echo $row['num_facture']; ?></p>
                <p><strong>Date de la facture:</strong> <?php echo date('d/m/Y', strtotime($row['date_facture'])); ?>
                </p>
                <p><strong>Télécharger:</strong> <a
                        href="../upload/<?php echo htmlspecialchars($row['pj_facture'], ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars($row['num_facture'], ENT_QUOTES, 'UTF-8'); ?>.pdf</a>
                </p>
            </div>
            <div class="alert alert-light" role="alert">
                <h5 id="list-item-2">Information sur LP1</h5>
                <hr>
                <?php if(!empty($id_lp1_info)){ ?></p>
                <p><strong>Numréro :</strong><?php echo $num_lp1_suivis; ?></p>
                <p><strong>Date:</strong><?php echo date('d/m/Y', strtotime($date_lp1));?></p>
                <p><strong>Quantité:</strong><?php echo $quantite_init." ".$unite_lp1; ?></p>
                <p><strong>Télécharger:</strong> <a
                        href="../upload/<?php echo htmlspecialchars($pj_lp1, ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars($num_lp1_suivis, ENT_QUOTES, 'UTF-8'); ?>.pdf</a>
                </p>
                <?php } else {?></p>
                <p><strong>Numréro :</strong><?php echo $numero_lp1; ?></p>
                <p><strong>Quantite
                        :</strong><?php echo $row3['quantite_lp1_initial_lp1_suivis'].' '.$row3['unite_substance_lp1']; ?>
                </p>
                <p><strong>Télécharger:</strong> <a
                        href="../upload/<?php echo htmlspecialchars($scan_lp1, ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars($numero_lp1, ENT_QUOTES, 'UTF-8'); ?>.pdf</a>
                </p>
                <?php } ?></p>
            </div>
            ...
            <div class="alert alert-light" role="alert">
                ...
                <hr>
                <p><strong>Nom:</strong> <?php echo $row['nom_porteur'].' '.$row['prenom_porteur']; ?></p>
            ...
            <div class="alert alert-light" role="alert">
                ...
                for ($i = 0; $i < count($afficheWord_brute); $i++) { echo $afficheWord_brute[$i] .', ';
                    ...
                    for ($i = 0; $i < count($afficheWord); $i++) { if($i==count($afficheWord) - 1){ echo
                        ...
                        ?><br>
                <p><br><strong>Numréro du CC:</strong><?php echo $row2['num_cc']; ?><strong>
                        du:</strong><?php echo date('d/m/Y', strtotime($row['date_cc'])); ?></p>
                <p><strong>Télécharger:</strong><a
                        href="../upload/<?php echo htmlspecialchars($row['pj_cc'], ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars($row['num_cc'], ENT_QUOTES, 'UTF-8'); ?>.pdf</a>
                </p>
            </div>
        </div>
        <div class="info2">
            <div class="alert alert-light" role="alert">
                <?php
                        $pdfFilePath = $row['scan_visa'];
                        if(empty($pdfFilePath)){
                            echo '<p class="alert alert-info">Aucun scan du visa.</p>';
                        }else{
