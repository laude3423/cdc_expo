...
$edit_societe_id = isset($_GET['edit_id']) ? $_GET['edit_id'] : null;
    ...
    if (isset($_POST['submit'])) {
        $nom = $_POST['nom'];
        $lieu_depart = $_POST['lieu'];
        $id_fret = $_POST['id'];
        if (empty($id_fret)) {
            // Insertion d'une nouvelle société
            $sql = "INSERT INTO `fret`(`nom_fret`, `lieu_depart) VALUES ('$nom', '$lieu_depart')";
            ...
            $sql = "UPDATE `fret` SET `nom_fret`='$nom', `lieu_depart`='$lieu_depart' WHERE id_fret='$id_fret'";
            $result = mysqli_query($conn, $sql);
...
<body>
    <div class="container">
        ...
        <table id="agentTable" class="table table-hover text-center">
            ...
            <tbody>
                <?php 
                $sql="SELECT * FROM `fret`";
                $result= mysqli_query($conn, $sql);
                while($row = mysqli_fetch_assoc($result)){
                  ?>
                <tr>
                    <td>✅</td>
                    <td><?php echo $row['nom_fret'] ?></td>
                    <td><?php echo $row['lieu_depart'] ?></td>
                    <td>
                        <a href="#" class="link-dark" onclick="openModal(<?php echo $row['id_fret']?>)"><i
                                class="fa-solid fa-pen-to-square me-3"></i></a>
                        <a href="#" class="link-dark" onclick="confirmerSuppression(<?php echo $row['id_fret']?>)"><i
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="staticBackdropLabel">Nouvel fret</h1>
                ...
                <div class="modal-body">
                    <form action="" method="post">
                        <div class="mb-3">
                            <label for="nom" name="nom" class="col-form-label">Nom du fret:</label>
                            <input type="text" class="form-control" name="nom" id="nom" placeholder="Nom du fret"
                                required style="font-size:90%">
                            <input type="hidden" id="id" name="id">
                        </div>
                        <div class="mb-3">
                            <label for="lieu" name="lieu" class="col-form-label">Lieu de départ:</label>
                            <input type="text" class="form-control" name="lieu" id="lieu" placeholder="Lieu de départ"
                                required style="font-size:90%">
                        </div>
    ...
    function openModal(edit_id = null) {
        ...
        if (edit_id) {
            ...
            document.getElementById('staticBackdropLabel').innerText = 'Modifier fret';
            var id = edit_id;
            ...
            function getDataById(id) {
                $.ajax({
                    ...
                    success: function(data) {
                        $('#id').val(data.id_fret);
                        $('#nom').val(data.nom_fret);
                        $('#lieu').val(data.lieu_depart);
                    },
                    error: function(xhr, status, error) {
            ...
            document.getElementById('staticBackdropLabel').innerText = 'Nouvel fret';
